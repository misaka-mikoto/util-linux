				...
				struct gpt_header *header, struct gpt_entry *ents)
{
	...
	if (offset != lseek(cxt->dev_fd, offset, SEEK_SET))
		goto fail;
	if (totwrite == write(cxt->dev_fd, ents, totwrite))
		return 0;
fail:
	return -errno;
}
...
static int gpt_verify_disklabel(struct fdisk_context *cxt)
{
	...
	if (ptnum) {
		nerror++;
		fdisk_warnx(cxt, _("Partition %ld overlaps with partition %ld."), ptnum, ptnum+1);
	}

	ptnum = partition_check_too_big(gpt->pheader, gpt->ents, cxt->total_sectors);
	if (ptnum) {
		nerror++;
		fdisk_warnx(cxt, _("Partition %ld is too big for the disk."), ptnum);
	}

	ptnum = partition_start_after_end(gpt->pheader, gpt->ents);
	if (ptnum) {
		nerror++;
		fdisk_warnx(cxt, _("Partition %ld ends before it starts."), ptnum);
	}
	...
	if (!nerror) { /* yay :-) */
		...
		fdisk_info(cxt, _("Using %d out of %d partitions."),
